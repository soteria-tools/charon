...
//! `transmute::<*T, usize>(p)` that feeds a `switch [0, otherwise]` into a call to the `ZeroIfNull`
//! builtin (the shape rustc emits when lowering pointer null-checks).
...
// the address-erasing builtin would change the observable result.

...
pub struct CliOpts {
    ...
    /// (e.g. `<*const T>::is_null`) with a call to the `ZeroIfNull` builtin.
    #[clap(long)]
    #[serde(default)]
...
pub struct TranslateOptions {
    ...
    /// Replace `if x { panic() }` with `assert(x)`.
    pub reconstruct_asserts: bool,
    /// Replace the `transmute::<*T, usize>(p)` that rustc emits for pointer null-checks with a call
    /// to the `ZeroIfNull` builtin.
    pub reconstruct_null_checks: bool,

...
impl<'tcx> ItemTransCtx<'tcx, '_> {
    ...
    pub fn translate_fn_pointer_method(
        ...
        let method_ref = {
            ...
            let trait_args = self_predicate.trait_ref.rustc_args(hax_state);
        ...
        let hax::FullDefKind::AssocFn { sig, .. } = method_def.kind() else {
            unreachable!()
        };
        // We only generate this item in mono mode, where regions are erased anyway.
        let signature = {
            let bound_sig = self
                .translate_region_binder(span, sig, |ctx, sig| ctx.translate_fun_sig(span, sig))?;

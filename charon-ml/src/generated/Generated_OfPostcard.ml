...
and generic_args_of_postcard (ctx : of_postcard_ctx) (st : postcard_state) :
  ...
  combine_error_msgs st __FUNCTION__
    (let* regions =
       indexed_map_of_postcard region_id_of_postcard region_of_postcard ctx st
     in
     let* types =
       indexed_map_of_postcard type_var_id_of_postcard ty_of_postcard ctx st
     in
     let* const_generics =
       indexed_map_of_postcard const_generic_var_id_of_postcard
         constant_expr_of_postcard ctx st
     in
     let* trait_refs =
       indexed_map_of_postcard trait_clause_id_of_postcard trait_ref_of_postcard
         ctx st
     in
...
and generic_params_of_postcard (ctx : of_postcard_ctx) (st : postcard_state) :
  ...
  combine_error_msgs st __FUNCTION__
    (let* regions =
       indexed_map_of_postcard region_id_of_postcard region_param_of_postcard
         ctx st
     in
     let* types =
       indexed_map_of_postcard type_var_id_of_postcard type_param_of_postcard
         ctx st
     in
     let* const_generics =
       indexed_map_of_postcard const_generic_var_id_of_postcard
     ...
     in
     let* trait_clauses =
       indexed_map_of_postcard trait_clause_id_of_postcard
         trait_param_of_postcard ctx st
     in
     let* regions_outlive =
       list_of_postcard
     ...
     let* trait_type_constraints =
       indexed_map_of_postcard trait_type_constraint_id_of_postcard
  ...
  combine_error_msgs st __FUNCTION__
    (let* __tag = int_of_postcard ctx st in
     ...
     | 2 ->
         ...
         in
         Ok (TargetDispatch x_0)
     | 3 -> Ok TraitMethodWithoutDefault
     | 4 -> Ok Opaque
     | 5 -> Ok Missing
     | 6 ->

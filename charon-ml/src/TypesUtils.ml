...
let ty_as_opt_adt (ty : ty) : (type_decl_id * generic_args) option =
  match ty with
  | TAdt (tref, _) -> Some (tref.id, tref.generics)
  | _ -> None
...
let ty_as_builtin_adt_opt (ty : ty) : (builtin_ty * generic_args) option =
  match ty with
  | TAdt (tref, Some id) -> Some (id, tref.generics)
  | _ -> None
...
let ty_as_custom_adt (ty : ty) : TypeDeclId.id * generic_args =
  match ty with
  | TAdt ({ id; generics }, None) -> (id, generics)
  | _ -> raise (Failure "Unreachable")
...
let ty_as_opt_box (box_ty : ty) : ty option =
  match box_ty with
  | TAdt ({ generics = { types = [ boxed_ty ]; _ }; _ }, Some TBox) ->
      Some boxed_ty
  | _ -> None
...
let mk_box_ty (box_id : type_decl_id) (ty : ty) : ty =
  TAdt ({ id = box_id; generics = mk_generic_args_from_types [ ty ] }, Some TBox)

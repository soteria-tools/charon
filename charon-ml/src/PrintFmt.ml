...
and pp_constant_expr (env : fmt_env) (fmt : Format.formatter)
  ...
  | CAdt (variant_id, fields) -> begin
      match cv.ty with
      | TAdt (tref, builtin) ->
          pp_const_aggregate env tref builtin variant_id fmt fields
...
and pp_ty (env : fmt_env) (fmt : Format.formatter) (ty : ty) : unit =
  ...
  | TAdt (tref, Some TTuple)
    when tref.generics.types <> []
         && not (type_decl_ref_is_specialized env tref) ->
  ...
  | TAdt (_, Some TStr) -> pp_string fmt "str"
  | TAdt (tref, _) -> pp_type_decl_ref env fmt tref
...
let rec pp_projection_elem (env : fmt_env) (sub : string)
  ...
  | Field (adt_id, opt_variant_id, fid) -> (
      let field_name =

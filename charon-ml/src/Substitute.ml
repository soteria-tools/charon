...
(** The field types of a tuple. This can either be the generics of the type (for
    polymorphic code), the fields themselves (for monomorphic code), or the
    fields after substitution (for partial monomorphization). Errors if [ty] is
    not a tuple! *)
let ty_as_tuple_fields (type_decls : type_decl TypeDeclId.Map.t) (ty : ty) :
    ty list option =
  let tref =
    match ty with
    | TAdt (tref, Some TTuple) -> tref
  ...
  | Some decl when not (type_decl_is_specialized decl) ->
      (* The polymorphic, one-per-arity declaration takes the field types as
             its arguments. *)
      Some tref.generics.types
  | Some { kind = Struct fields; generics = params; _ } ->
      if tref.generics = empty_generic_args then
        (* Monomorphized whole: the declaration's fields are already the
               concrete types. *)
        Some (List.map (fun f -> f.field_ty) fields)
      else
        (* Specialized for one instantiation but still generic: the
               arguments are the parameters that survived, so the declaration is
               the only place the fields are written down. *)
        let subst =
          subst_item_vars
            (make_sb_subst_from_generics params tref.generics Self)
        in
